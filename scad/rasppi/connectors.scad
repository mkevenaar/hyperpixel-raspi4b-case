...
module usb(ver=4)
{
    if (ver == 4) {
        ...
        move([70.5,21.6,0]) usb_connector(false);
        move([70.5,39.6,0]) usb_connector(false);
    }
}

module ethernet(ver=4)
{
    if (ver == 4) {
        move([66.5,38,0]) {
            color("silver") {
...
module powerin(ver=4)
{
    if(ver == 4) {
        ...
        color("silver") difference() {
            union() {
                move([6.5,-1,0]) cuboid([8,6,2.6], center=false, fillet=0.25, chamfer=1, edges=EDGE_BOT_LF+EDGE_BOT_RT); // Body
                move([6.75,-1.5,0]) cuboid([8,0.5,2.6], center=false, fillet=0.25, chamfer=1, edges=EDGE_BOT_LF+EDGE_BOT_RT); // Lip
            }

            move([7,-2,0.25]) cuboid([7.5,6,2.6 - 0.5], center=false, fillet = 0.25, chamfer=1, edges=EDGE_BOT_LF+EDGE_BOT_RT);
        }

        color([0.1,0.1,0.1]) {
            move([7.50,-2,1.25]) cuboid([6.5,6,1.2 - 0.5], center=false, fillet = 0.25);
        }
    }
}

module hdmi_connector(ver=4)
{
    if (ver==4) {
        color("silver") difference() {
            union() {
        ...
        color("silver") difference() {
            union() {
                move([0,-1,0.5]) cuboid([16,11.5,6], center=false, fillet=0.25, chamfer=2, edges=EDGE_BOT_LF+EDGE_BOT_RT); // Body
                move([0,-1.5,0.5]) cuboid([16,0.5,6], center=false, fillet=0.25, chamfer=2, edges=EDGE_BOT_LF+EDGE_BOT_RT); // Lip
            }

            move([0.25,-2,0.75]) cuboid([15.5,11.5,6 - 0.5], center=false, fillet = 0.25, chamfer=2, edges=EDGE_BOT_LF+EDGE_BOT_RT);
        }

        color([0.1,0.1,0.1]) {
            move([1,0.5,3]) cuboid([14,6,1], center=false, fillet = 0.25);
        }
    }
...
{
    if (ver==4) {
        move([23,-1,0]) hdmi_connector(ver);
        move([36.25,-1,0]) hdmi_connector(ver);
    }
    else if (ver==3) {
        move([24,-1,0]) hdmi_connector(ver);
    }
}
